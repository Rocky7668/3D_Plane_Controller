...
namespace Oyedoyin.Common
{
    ...
    public class Controller : MonoBehaviour
    {
        ...
        {
            Updown = value.ReadValue<Vector2>();
            if (isVeticalMove)
            {
                m_input._rawPitchInput = Updown.y;
                print("m_input._rawPitchInput ==== " + Updown.y);
            }
        }

        public Vector2 onleftright;
        public void Onroll(InputAction.CallbackContext value)
        {
            onleftright = value.ReadValue<Vector2>();
            if (isHorizontalMove)
            {
                m_input._rawRollInput = onleftright.x;
                print("m_input._rawRollInput ==== " + onleftright.x);
            }
        }

        public Vector2 throttleLever;
        public void OnThrottel(InputAction.CallbackContext value)
        {
            throttleLever = value.ReadValue<Vector2>();

            print("throttleLever ------------ " + throttleLever);
        }
        ...
        // Common
        public void OnStartEngineGlobal() { if (m_inputType == InputType.Default && m_inputLogic == InputLogic.InputSystem) { Debug.Log("Engines Turen on"); TurnOnEngines(); } }
        public void OnStopEngineGlobal() { if (m_inputType == InputType.Default && m_inputLogic == InputLogic.InputSystem) { Debug.Log("Engines Turen off"); TurnOffEngines(); } }
    ...
    public class SceneAutopilot
    {
        ...
        {
            Debug.Log("Autopilot Toggle Enable");
        ...
        {
            Debug.Log("Autopilot Toggle Disable");
