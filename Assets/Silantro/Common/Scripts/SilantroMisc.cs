...
namespace Oyedoyin.Common
{
    ...
    public class SilantroMisc : MonoBehaviour
    {
        ...
        {
            m_display.m_vehicle.isHorizontalMove = false;
            m_display.m_vehicle.isVeticalMove = false;
            m_display.m_vehicle.m_sceneAutopilot.EnableAutopilot();
        ...
        private void FixedUpdate()
        {
            ...
            if (isFly)
            {
                if (m_display.altitudeCount > m_altitudeStep  +  DifferentDistance || m_display.altitudeCount < m_altitudeStep - DifferentDistance)
                {
                    Debug.Log("<color=cyan> ---------------- Game Over ----------------  </color>");
                    m_display.m_GameOver.SetActive(true);
#if UNITY_EDITOR
                    ...
                    {
                        m_display.m_vehicle.OnResetScene();
                    });*/
                }
            }
            else
            {
                m_display.m_vehicle.throttleLever.y = 1;
                print("m_display.m_vehicle.throttleLever.y = 1");
            }
        ...
        public int time;
        private Oyedoyin.Common.Controller m_vehicle;
        public void Timer()
        {
            time++;

            Debug.Log("<color=red> Timer -------- " + time + "</color>");


            if (m_display.altitudeCount >= m_altitudeStep)
            {
                CancelInvoke("Timer");
                Time.timeScale = 1;
                m_display.m_vehicle.isHorizontalMove = true;
                m_display.m_vehicle.isVeticalMove = true;
                m_display.m_loading.t.Kill();
                m_display.m_loading.LoadingBar.value = 1;
                m_display.m_loading.gameObject.SetActive(false);
                DOTween.Sequence().AppendInterval(2f).OnComplete(() =>
                {
                    isFly = true;
                });
                m_display.m_vehicle.m_sceneAutopilot.DisableAutopilot();
            }
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(control.FindPropertyRelative("m_type"), new GUIContent("Mode"));
                ...
                if (extension.m_display.displayPoints)
                {
                    ...
                    GUILayout.Space(3f);
                    extension.m_display.m_loading = EditorGUILayout.ObjectField("Loading", extension.m_display.m_loading, typeof(Loading), true) as Loading;
                    GUILayout.Space(3f);
                    extension.m_display.m_GameOver = EditorGUILayout.ObjectField("GameOver", extension.m_display.m_GameOver, typeof(GameObject), true) as GameObject;
                }
            }
